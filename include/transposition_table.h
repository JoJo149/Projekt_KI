...
namespace TT {
    ...
    struct TT_Move {
        ...
        explicit TT_Move(const Move& move)
         : from_sq(std::countr_zero(move.from)), to_sq(std::countr_zero(move.to)), move_distance(move.move_distance) {assert(move.from != 0 && move.to != 0);}
        ...
        bool operator==(const TT_Move& other) const {
