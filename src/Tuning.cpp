...
void startPythonServer() {
    ...
    int result = std::system("python3 ../server/server.py");
...
void setupPythonEnvironment() {
    ...
    int status = std::system("../server/tuning_run_server.sh");
    if (status != 0) {
        std::cerr << "Failed to install Python dependencies!" << std::endl;
    } else {
        std::cout << "Python dependencies installed successfully." << std::endl;
    }
}
*/
